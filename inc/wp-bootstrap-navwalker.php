		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			if ( 0 === strcasecmp( $item->attr_title, 'divider' ) && 1 === $depth ) {
                ...
                if ( $args->has_children ) {
                    // Nível raiz usa 'dropdown', níveis internos usam 'dropdown-submenu'
                    $class_names .= ( 0 === $depth ) ? ' dropdown' : ' dropdown-submenu';
                }
                ...
                if ( $args->has_children && 0 === $depth ) {
                    ...
                    // Submenu de segundo nível (e além): usar toggle também
                    $atts['href']             = '#';
                    $atts['data-bs-toggle']   = 'dropdown';
                    $atts['data-bs-auto-close'] = 'outside';
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'edit_theme_options' ) ) {
				...
				$output     .= '<ul' . ( $menu_id ? ' id="' . esc_attr( $menu_id ) . '"' : '' ) . ( $menu_class ? ' class="' . esc_attr( $menu_class ) . '"' : '' ) . '>';
					$output .= '<li><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" title="">' . esc_html__( 'Add a menu', 'f2fdashboard' ) . '</a></li>';
				$output     .= '</ul>';
				if ( $container ) {
